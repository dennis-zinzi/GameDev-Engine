...
void	OBJMesh::SetTexturesFromMTL(string &mtlFile, string &mtlType) {
	...
	if(i != materials.end()) {
		if(!i->second.diffuse.empty())	{
			i->second.diffuseNum = i->second.diffuseNum;
		}
#ifdef OBJ_USE_TANGENTS_BUMPMAPS
		if(!i->second.bump.empty())	{
			i->second.bumpNum = i->second.bumpNum;
		}
#endif
		return;
	}

	std::ifstream f(string(MESH_PATH + mtlFile).c_str(),std::ios::in);
	...
	while(!f.eof()) {
		...
		if(currentLine == MTLNEW) {
			...
			if(!currentMTL.diffuse.empty()) {
				currentMTL.diffuseNum = SOIL_load_OGL_texture(string(TEXTURE_PATH + currentMTL.diffuse).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS);
			}
		}
			...
			if(!currentMTL.bump.empty()) {
				currentMTL.bumpNum = SOIL_load_OGL_texture(string(TEXTURE_PATH + currentMTL.bump).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS);
			}
		}
	}
...
void	OBJMesh::FixTextures(MTLInfo &info) {
	if(!info.bumpNum) {
		...
		info.bumpNum = SOIL_load_OGL_texture(string(TEXTURE_PATH + info.bump).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y |  SOIL_FLAG_TEXTURE_REPEATS);
	}
}
//#endif